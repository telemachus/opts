...
passed to NewGroup becomes the [Name] that the Group reports.  As an example,
we will create an application that validates and optionally corrects the naming
convention  used in one or more files.
...
associated with those options can be trusted for use.  Parse should receive a slice of strings containing everything on the command line other than the name of the program.  In other words, you want to pass os.Args[1:], though you may want to tweak those values before you pass them.
*/
package opts
